...
async function showPosition(position){
    ...
    try{
        const currentLocation = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=YOUR_API_KEY`);
...
async function fetchData(){
    try{
        const currentWeather = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=YOUR_API_KEY`);
        ...
        //Five Day Forecast
        const fiveDayWeather = await fetch(`https://api.openweathermap.org/data/2.5/forecast?id=${id}&appid=YOUR_API_KEY`);
        ...
        const airQaulityIndex = await fetch(`http://api.openweathermap.org/data/2.5/air_pollution?lat=${coord.lat}&lon=${coord.lon}&appid=YOUR_API_KEY`)
